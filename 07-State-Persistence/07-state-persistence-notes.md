...
- `ReadWriteOnce`: Only one node (thus pods on that node) can mount the volume for read/write at a time. Used when your application (e.g. database) needs read/write access and won’t be replicated across nodes.
